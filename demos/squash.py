...
if __name__ == "__main__":
    ...
    if not osp.isfile(VIDEO_NAME):
        try:
            ...
            gdown.download(
                "https://drive.google.com/uc?id=1S46QDdv0pnU98mNXxechvIVK8qOBFlkl"
            )
    ...
    dataset = LoadImages(
        "./{}".format(VIDEO_NAME), img_size=imgsz, stride=stride, auto=True
    )
    ...
    if not args.display:
        writer = cv2.VideoWriter(
            video_name + ".mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h)
        )
    ...
    for i, (path, img, im0, _) in enumerate(dataset):
        ...
        bboxes, poses, _, _, _ = post_process_batch(
        ...
        for j, (bbox, pose) in enumerate(zip(bboxes, poses)):
            ...
            if size < CROWD_THRES:
                cv2.rectangle(
                    im0_copy, (int(x1), int(y1)), (int(x2), int(y2)), GRAY, thickness=2
                )
                for x, y, _ in pose[:5]:
                    cv2.circle(
                        im0_copy, (int(x), int(y)), CROWD_KP_SIZE, GRAY, CROWD_KP_THICK
                    )
        ...
        def draw_player_poses(im0, missing=-1):
            for j, (bbox, pose, color) in enumerate(
                zip(
                ...
                im0 = cv2.addWeighted(
                    im0, PLAYER_ALPHA_BOX, im0_copy, 1 - PLAYER_ALPHA_BOX, gamma=0
                )
                im0_copy = im0.copy()
                for x, y, _ in pose:
                    cv2.circle(
                ...
                im0 = cv2.addWeighted(
                    im0, PLAYER_ALPHA_POSE, im0_copy, 1 - PLAYER_ALPHA_POSE, gamma=0
                )
            return im0
        ...
        if i == 0:
            ...
            for pose in poses_last:
                dist.append(
                    np.mean(
                        np.linalg.norm(player_poses[0, :, :2] - pose[:, :2], axis=-1)
                    )
                )
            if np.argmin(dist) == 0:
                orange_player = 0
            ...
            if len(player_poses) == 1:
                if orange_player == 0:  # missing blue player
                    player_poses = np.concatenate(
                        (player_poses, poses_last[1:]), axis=0
                    )
                    player_bboxes = np.concatenate(
                        (player_bboxes, bboxes_last[1:]), axis=0
                    )
                    missing = 1
        ...
        if args.gif:
            gif_frames.append(
                cv2.resize(im0, dsize=None, fx=0.25, fy=0.25)[:, :, [2, 1, 0]]
            )
