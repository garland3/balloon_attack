...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--color", type=int, nargs="+", default=[255, 255, 255], help="pose color"
    )
    ...
    parser.add_argument(
        "--end", type=int, default=42, help="end time (s), -1 for remainder of video"
    )
    ...
    parser.add_argument(
        "--kp-obj", action="store_true", help="plot keypoint objects only"
    )
    ...
    if write_video:
        writer = cv2.VideoWriter(
            out_path + ".mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h)
        )
    ...
    for i, (path, img, im0, _) in enumerate(dataset):
        ...
        if not args.gif and args.fps_size:
            cv2.putText(
                im0,
                "{:.1f} FPS".format(1 / t),
                (5 * args.fps_size, 25 * args.fps_size),
        ...
        if args.gif:
            gif_img = cv2.cvtColor(
                cv2.resize(im0, dsize=tuple(args.gif_size)), cv2.COLOR_RGB2BGR
            )
            if args.fps_size:
                cv2.putText(
                    gif_img,
