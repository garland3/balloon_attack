...
class Detect(nn.Module):
    ...
    def __init__(
        self, nc=80, anchors=(), ch=(), inplace=True, num_coords=0
    ):  # detection layer
        super().__init__()
...
class Model(nn.Module):
    def __init__(
        ...
        if nc + num_coords and nc + num_coords != self.yaml["nc"]:
            LOGGER.info(
                f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc + num_coords}"
            )
    ...
    def _descale_pred(self, p, flips, scale, img_size, kp_flip):
        ...
        if self.inplace:
            ...
            if flips == 2:
                ...
                if kp_flip:
                    p[..., 6 : 6 + self.nc - 1] = p[..., 6 : 6 + self.nc - 1][
                        ..., kp_flip
                    ]  # de-flip bbox conf
                    p[..., -self.num_coords :: 2] = (
                        img_size[1] - p[..., -self.num_coords :: 2]
                    )  # de-flip kp x
                    p[..., -self.num_coords :: 2] = p[..., -self.num_coords :: 2][
                        ..., kp_flip
                    ]  # swap lr kp (x)
                    p[..., -self.num_coords + 1 :: 2] = p[
                        ..., -self.num_coords + 1 :: 2
                    ][
                        ..., kp_flip
                    ]  # swap lr kp (y)
