...
def train(
    hyp, opt, device, callbacks=Callbacks()  # path/to/hyp.yaml or hyp dictionary
):
    (
        save_dir,
        epochs,
    ...
    results = (
        ...
        0,
        0,
    )  # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls, kp)
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--weights", type=str, default="yolov5s6.pt", help="initial weights path"
    )
    ...
    parser.add_argument(
        "--autobalance",
        action="store_true",
        help="Learn keypoint and object loss scaling (experimental)",
    )
