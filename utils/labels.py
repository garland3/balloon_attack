...
def write_kp_labels(data):
    assert not osp.isdir(
        osp.join(data["path"], data["labels"])
    ...
    splits = [
        osp.splitext(osp.split(data[s])[-1])[0]
        for s in ["train", "val", "test"]
        if s in data
    ]
    annotations = [
        osp.join(data["path"], data["{}_annotations".format(s)])
        for s in ["train", "val", "test"]
        if s in data
    ]
    test_split = [
    ...
    for split, annot, is_test in zip(splits, annotations, test_split):
        ...
        labels_path = osp.join(
            data["path"], "{}/{}".format(data["labels"], split if is_coco else "")
        )
        if not is_test:
        ...
        if not is_test:
            ...
            for id in pbar:
                ...
                with open(
                    osp.join(
                        labels_path,
                        "{}.txt".format(osp.splitext(img_info["file_name"])[0]),
                    ),
                    "a",
                ) as f:
                    # write person object
