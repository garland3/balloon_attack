...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
            ...
            if n:
                ...
                if self.num_coords:
                    ...
                    if len(tkp_vis):
                        ...
                        pkp = (pkp.sigmoid() * 4.0 - 2.0) * anchors[i][
                            :, None, :
                        ]  # range [-2, 2] * anchor
                        pkp_vis = pkp[vis]
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(
                        ps[:, 5 : 5 + self.nc], self.cn, device=device
                    )  # targets
        ...
        if self.autobalance:
            loss = (lbox + lobj + lcls) / (
                torch.exp(2 * self.loss_coeffs[0])
            ) + self.loss_coeffs[0]
    ...
    def build_targets(self, p, targets):
        ...
        gain = torch.ones(
            7 + self.num_coords * 3 // 2, device=targets.device
